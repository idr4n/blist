    ...
    <div>
      ...
      {bookmark && (
        <div className='flex flex-col max-w-[90%] lg:max-w-screen-md mx-auto'>
          <h3 className='text-center py-5'>Bookmark URL: {getURL(bookmark.title)}</h3>
          <div className='py-3 font-light text-gray-600'>
            <p className='py-1'><span className='font-bold'>Title</span>: {bookmark.title}</p>
            <p className='py-1'><span className='font-bold'>Tags</span>: {bookmark.tags}</p>
          </div>
        </div>
      )}
    </div>
